<script>
  ...
  const handleEdit = (row) => {
    window.location.href = `${edit_link}/${row.id}`;
  };
...
<div class="items-center mb-4">
  <ButtonLink label={"Create"} {create_url}></ButtonLink>
</div>
    ...
    <tr>
      {#each columns as column}
        <th
          ...
          on:click={() => sortData(column.field)}
        >
          <p
          ...
          <td class="py-3 px-5 border-b border-blue-gray-50">
            {#if column.field === "completion"}
              <div class="w-10/12">
                ...
                >
                  <div
                    class="flex justify-center items-center h-full bg-gradient-to-tr {row[
                      column.field
                    ] == 100
                      ? 'from-green-600 to-green-400'
                      : 'from-blue-600 to-blue-400'} text-white"
                    style="width: {row[column.field]}%;"
                  ></div>
                </div>
              </div>
            {:else}
              <p
                class="block antialiased font-sans {column.field === 'name'
                  ? 'text-sm leading-normal text-blue-gray-900 font-bold'
                  : 'text-xs font-medium text-blue-gray-600'}"
              >
                {row[column.field]}
              </p>
            {/if}
